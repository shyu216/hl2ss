...
bool Channel_MQX::Dispatch()
{
    ...
    {
    Sleep(1);
    return WaitForSingleObject(m_event_client, 0) == WAIT_TIMEOUT;
    }

    ok = recv_u32(m_socket_client, m_event_client, id); // block
    if (!ok) { return false; }
...
{
    while (Dispatch());
    if (!Stopping()) { MessageQueue_Client_WaitForRestart(); } // block
}
