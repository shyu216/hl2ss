...
void Channel_MQ::Loop_RX()
{
    ...
    do
    {
    if (!Server_ReadAvailable(m_socket_client))
    {
    Sleep(1);
    continue;
    }

    ok = recv_u32(m_socket_client, m_event_client, command); // block
    if (!ok) { break; }
    ok = recv_u32(m_socket_client, m_event_client, size); // block
    if (!ok) { break; }
...
void Channel_MQ::Run()
{
    ...
    if (!Stopping()) { MessageQueue_Server_WaitForRestart(); } // block
}
