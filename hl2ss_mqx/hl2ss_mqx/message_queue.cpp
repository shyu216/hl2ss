...
static std::queue<MQ_Item*> g_queue_si;
...
{
    while (!g_queue_si.empty()) { MessageQueue_Item_Delete(pull(g_queue_si)); }
    while (!g_queue_so.empty()) { pull(g_queue_so); }
...
{
    CriticalSection cs(&m_lock_si);
    return g_queue_si.empty() ? MQ_MARKER : g_queue_si.front()->size;
}
...
void MessageQueue_Server_Restart()
{
    {
    CriticalSection cs(&m_lock_so);
    while (WaitForSingleObject(m_semaphore_so, 0) == WAIT_OBJECT_0) { g_queue_so.pop(); }
    }
...
{
    while (!g_queue_ci.empty()) { MessageQueue_Item_Delete(pull(g_queue_ci)); }
    while (!g_queue_co.empty()) { pull(g_queue_co); }
